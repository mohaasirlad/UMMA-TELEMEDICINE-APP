...
public class DoctorMessagesFragment extends Fragment {
    ...
    private void loadMessages() {
        Executors.newSingleThreadExecutor().execute(() -> {
            ...
            List<String> dbConversationIds = chatMessageDao.getAllConversationIdsForUser(currentUserId);

            for (String conversationId : dbConversationIds) {
                ChatMessage lastDbMsg = chatMessageDao.getLastMessageForConversation(conversationId);
                int unreadCount = chatMessageDao.getUnreadCountForConversation(conversationId, currentUserId);
                ...
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        if (name != null) {
                            ...
                            if (isAdded()) {
                                loadMessages(); // Refresh UI with real name
                            }
                        }
                    }
